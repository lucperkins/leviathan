...
export const REF_KINDS = [
  ...
  { kind: "touchstone", dir: "src/content/touchstones" },
];

/**
 * Reads every ref collection and returns [{ kind, id, terms }] where `terms`
 * are the words that should be linked in chapter text. Falls back to the
 * entry title when no `terms` list is given.
 */
export function loadRefTerms(kinds = REF_KINDS) {
  return kinds.flatMap(({ kind, dir }) => {
 ...
 * <concept-ref data-kind="concept|touchstone" data-concept="id">…</concept-ref>.
 ...
 * edits to an entry's `terms` apply the next time a chapter is compiled.
 * Adding or removing an entry is handled by the ref-watch integration, which
 * invalidates compiled chapter modules so this plugin runs again.
 */
export default function rehypeConcepts() {
  const buildMatchers = () =>
    loadRefTerms()
