...
import { quoteQuery } from "../lib/text.mjs";

const fold = (s) => s.toLowerCase().replace(/[^a-z0-9]+/g, "");

/**
 * Primary-text pages: for each verbatim sentence in the frontmatter
 * `pullquotes` list, insert a pull quote <aside id="quote-N"> after the
 * top-level paragraph that contains it. The quote is a link to
 * ?hl=<opening words>#pN, which the Alpine `quoteTarget` component intercepts
 * to highlight the sentence in place. Must run after
 * rehype-chapter-paragraphs (needs the paragraph ids); nested inside <aside>,
 * the quote's <p> is not numbered.
 *
 * @param {{ textCollection: string }} options
 */
export default function rehypePullquotes({ textCollection }) {
  ...
  return (tree, file) => {
    ...
    for (const quote of quotes) {
      ...
      // same-page link: only the query and fragment
      const href = quoteQuery(Number(paraId.slice(1)), quote);
