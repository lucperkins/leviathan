...
/** Storage keys are namespaced by the site config's `storagePrefix`. */
const key = (k: string) => `${config.storagePrefix}:${k}`;
...
/** Where the reader last was, kept in localStorage under `<prefix>:reading`. */
interface Reading {
  /** Unit id, e.g. "13-of-the-naturall-condition-of-mankind". */
  id: string;
...
export default (Alpine: Alpine) => {
  ...
  Alpine.data("navSection", (id: string) => ({
    open: Alpine.$persist(false).as(key(`nav:${id}`)),
    init() {
  ...
  const theme: ThemeStore = {
    stored: Alpine.$persist(null as Theme | null).as(key("theme")),
  ...
  Alpine.data("readingPosition", () => ({
    saved: Alpine.$persist(null as Reading | null).as(key("reading")),
    timer: undefined as number | undefined,
    init() {
  ...
  Alpine.data("resumeReading", () => ({
    saved: Alpine.$persist(null as Reading | null).as(key("reading")),
    get href() {
      if (!this.saved) return `${config.text.basePath}/${config.text.firstUnitId}/`;
      return `${config.text.basePath}/${this.saved.id}/${this.saved.para ? `#${this.saved.para}` : ""}`;
    },
    get label() {
  ...
  /**
   * A figure with one region always shown: hovering or focusing another
   * previews it without losing the pinned one, so the panel never goes empty
   * and a click still works on a touch screen.
   */
  Alpine.data("pinnedFigure", () => ({
    pinned: 0,
  ...
  Alpine.data("navScroll", () => ({
    ...
    save() {
      ...
      requestAnimationFrame(() => {
        this.pending = false;
        try {
          sessionStorage.setItem(key("nav-scroll"), String(this.$el.scrollTop));
        } catch {}
      });
    },
  }));
